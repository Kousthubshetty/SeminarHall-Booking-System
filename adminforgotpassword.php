...
if(isset($_POST['btnsubmit']))
{
    ...
    $sqllogin= "SELECT * FROM admin WHERE emailid='$_POST[emailid]' AND status='Active'";
	$qsqllogin = mysqli_query($con,$sqllogin);
	$sqllogin1= "SELECT * FROM admin WHERE emailid='$_POST[emailid]' AND status='Not_verified'";
    ...
    if(mysqli_num_rows($qsqllogin) == 1)
    {
		...
		$mailid = $rslogin['emailid'];
		$mailname = $rslogin['adminname'];
		$_SESSION['pwdsession'] = rand(111111,999999);
		$subject = "Password Recovery Mail form Seminar Hall Booking";
		$message ="Hello " . $rslogin['adminname'] . ",<br><br><b>Email ID/Login ID :- </b>" . $rslogin['emailid'] . "<br>";
		$tblmessage="";
		$arrdatalink="http".(!empty($_SERVER['HTTPS']) ? 's' : '') . "://" . $_SERVER['SERVER_NAME'].substr($_SERVER['PHP_SELF'], 0, strrpos($_SERVER['PHP_SELF'], '/')) . "/adminpasswordrecovery.php?pwdsession=$_SESSION[pwdsession]&id=$rslogin[0]";
